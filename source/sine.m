function [outdata, tbase, out_rate, err] = sine(sfile, comp_num)
% noise: Acquisition method to make sine wave.
% Usage
%     sine(sfile, comp_num).
...
if(isempty(sfile))
   QueMessage('sine: No STIM?', 1);
   return;
end;

possible_sequences = {'DC', 'Frequency', 'Amplitude'};
if(chklength(sfile, possible_sequences, [1,1,1]))  % make sure incoming arguments are acceptable
   return;
end;

[dc, freq, amp, errp] = chkpars(sfile.SeqParList.v, possible_sequences); % check the parameters to be sequenced to be sure they are valid also
if(errp)
   return;
end;
...
base_freq = sfile.Frequency.v; % get basic frequency list
...
if(comp_num > 0)
   ...
   QueMessage('sine: sine wave on the fly');
else
   ml = [1:nout]; % compute the whole thing...
   QueMessage('sine: sine wave batch computation');
end;
...
for m=ml % for all stimuli in the set (sequence)
   ...
   if(~isempty(freq)) % do we sequence the random number seed? (many different traces!)
      freqo = vlist{freq}(m);
   end;
   ...
   yn(1:maxpts)=sin([tbase{k}.v]*2*pi*freqo/1000); % normally distributed random noise, 0 mean amplitude 1
   ...
   yn(1:winl) = yn(1:winl).*w'; % reapply taper...
   yn(n1:n2) = yn(n1:n2) .*fliplr(w');
